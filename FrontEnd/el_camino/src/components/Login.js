...
import { useSession } from "../hooks/useSession"; // Importa el hook useSession
import { api } from "../services/api"; // Importa el módulo api
import BoxForm from "./BoxForm"; // Importa el componente BoxForm
  ...
  return (
    <Container className="d-flex flex-grow-1 justify-content-center align-items-center">
      <BoxForm handleSubmit={handleSubmit} dataForm={dataForm}>
        <Form.Text as="p" className={"d-block text-primary text-center fs-3 fw-bold mb-4"}>
          Iniciar sesión
        </Form.Text>
        ...
        {isError && (
  <Alert variant="danger" className="py-2 text-center">
    El e-mail o la contraseña son inválidos.
  </Alert>
)}
        ...
        <Form.Text className="text-center d-block text-secondary mt-3">
          ...
          <Link className="text-decoration-none font-500 text-primary" to="/register">
            {" "}
            Regístrate
          </Link>
        </Form.Text>
